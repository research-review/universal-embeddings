...
server {
  ...
  server_name embeddings.cc embeddings.cs.uni-xxx.de;
...
server {
  ...
  listen [::]:8443 ssl default_server;
  ssl_certificate /opt/cert/embeddings.cs.uni-xxx.de.pem;
  ssl_certificate_key /opt/cert/embeddings.cs.uni-xxx.de.key;
  location / { try_files $uri @embeddings; }
  location @embeddings {
    include uwsgi_params;
    uwsgi_pass unix:/tmp/embeddingscc.sock;
